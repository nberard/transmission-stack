...
if $NEED_SAMBA; then
    ...
    docker exec -it $SAMBA_CONTAINER_NAME USERS_DIR=$USERS_DIR/srv/add_user.sh -u $USERNAME -p $PASSWORD
fi
...
if $NEED_SUBTITLES; then
    ...
    tee -a .env << EOF
subfinder_login=berard.nicolas@gmail.com
subfinder_password=FEzLDIovArAPB6KnxodH
subfinder_user_agent=Touplitoui
